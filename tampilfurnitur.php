...
<br>
        ...
        if(isset($_POST['simpan'])){
          ...
          $stn_hrg = $_POST['stn_hrg'];
              ...
              if( $no_brg=='' || $merk=='' || $status=='' || $th_beli=='' || $jumlah=='' || $stn_hrg=='' )
              {
                ...
                $query1="SELECT * FROM kba_furnitur where no_brg = '$no_brg'";
                ...
                if ($aset->Insertfurnitur($idmodel,$no_brg, $model, $merk, $status, $tgl, $th_beli, $jumlah, $stn_hrg,$total))
                {
                  echo '<script>
 ...
 <?php 
              if($_SESSION['hak_akses']=='User'){
                ?>
            <button onclick="document.getElementById('tambahfurnitur').style.display='block'" class="w3-button w3-blue w3-right">Tambah Data</button>
            <?php
              }
?>
<div id="tambahfurnitur" class="w3-modal " style="margin-left:0px ">
  <div class="w3-modal-content">
    <header class="w3-container w3-teal"> 
      <span onclick="document.getElementById('tambahfurnitur').style.display='none'" 
            ...
            <form class="w3-container" method="POST" action="">
                ...
                <select class="form-select" name="no_barang" aria-label="Default select example">
                  ...
                  <?php
                    ...
                    $query="SELECT * FROM kba_furnitur ORDER BY id ASC";
                    $kba_furnitur->Tampilfurnitur($query);
                  ?>
                  </select>
              <p>      
...
<h3 style="margin-left: 30px">Data Furnitur</h3>
<div class="row">
<div class="col-lg-6 mt-sm-2 mb-lg-5" style="margin-left: 150px">
  ...
  <?php
      if(isset($_POST['print'])){
        $tahun = $_POST['tahun'];
        echo "<script>
            window.location='cetak_furnitur.php?tahun=$tahun';
        </script>";
      }
  ...
  <select class="form-select" name="tahun" onchange="kodebarang()" id="idkdbrg" aria-label="Default select example">
    ...
    <?php
      require_once 'oop/class_kelolabarangaset.php';
      $query="SELECT DISTINCT(YEAR (th_beli)) AS tahun FROM `crudfurnitur` ORDER BY tahun";
      $kba_furnitur->Tampiltahunfurnitur($query);
    ?>
  </select>
...
<?php 
  require_once 'oop/class_furnitur.php';
  $query = "SELECT * FROM crudfurnitur ORDER BY th_beli DESC";
  
  if($_SESSION['hak_akses'] == 'User'){
    $aset->Tampilfurnitur($query);
  }
  else if($_SESSION['hak_akses'] == 'admin'){
      $aset->Tampiladminfurnitur($query);
